...
public class ScreenShot : MonoBehaviour
{
    ...
    private string uniqueFilename(int width, int height)
    {
        ...
        if (folder == null || folder.Length == 0)
        {
            ...
            string mask = string.Format("screen_{0}_*.{1}", SceneManager.GetActiveScene().name, format.ToString().ToLower());
            counter = Directory.GetFiles(folder, mask, SearchOption.TopDirectoryOnly).Length;
        }

        // use width, height, and counter for unique file name
        var filename = string.Format("{0}/screen_{1}_{2}.{3}", folder, SceneManager.GetActiveScene().name, counter, format.ToString().ToLower());
